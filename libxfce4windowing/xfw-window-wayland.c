...
xfw_window_wayland_set_skip_pager(XfwWindow *window, gboolean is_skip_pager, GError **error) {
    g_set_error_literal(error, XFW_ERROR, XFW_ERROR_UNSUPPORTED, "Pager skipping is not supported on Wayland");
    return FALSE;
}

static gboolean
xfw_window_wayland_set_skip_tasklist(XfwWindow *window, gboolean is_skip_tasklist, GError **error) {
    g_set_error_literal(error, XFW_ERROR, XFW_ERROR_UNSUPPORTED, "Tasklist skipping is not supported on Wayland");
    return FALSE;
}
...
xfw_window_commit_changes(XfwWindowWayland *window) {
    ...
    if (pending->icon_changed) {
        ...
        g_list_free_full(window->priv->icon_sizes, g_free);
        window->priv->icon_sizes = NULL;

        window->priv->icon_name = pending->new_icon_name;
        pending->new_icon_name = NULL;
        window->priv->icon_sizes = pending->new_icon_sizes;
        pending->new_icon_sizes = NULL;
...
xfce_toplevel_no_icon(void *data, struct xfce_foreign_toplevel_handle_v1 *xfce_toplevel) {
    ...
    g_list_free_full(window->priv->pending.new_icon_sizes, g_free);
    window->priv->pending.new_icon_sizes = NULL;
    window->priv->pending.icon_changed = TRUE;
}
