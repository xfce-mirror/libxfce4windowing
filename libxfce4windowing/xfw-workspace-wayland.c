...
static void workspace_name(void *data, struct zcosmic_workspace_handle_v1 *workspace, const char *name);
static void workspace_coordinates(void *data, struct zcosmic_workspace_handle_v1 *workspace, struct wl_array *coordinates);
static void workspace_state(void *data, struct zcosmic_workspace_handle_v1 *workspace, struct wl_array *state);
static void workspace_capabilities(void *data, struct zcosmic_workspace_handle_v1 *workspace, struct wl_array *capabilities);
static void workspace_removed(void *data, struct zcosmic_workspace_handle_v1 *workspace);

static const struct zcosmic_workspace_handle_v1_listener workspace_listener = {
    .name = workspace_name,
    .coordinates = workspace_coordinates,
    .state = workspace_state,
    .capabilities = workspace_capabilities,
    .remove = workspace_removed,
};
...
xfw_workspace_wayland_constructed(GObject *obj) {
    ...
    zcosmic_workspace_handle_v1_add_listener(workspace->priv->handle, &workspace_listener, workspace);
}
...
xfw_workspace_wayland_finalize(GObject *obj) {
    ...
    zcosmic_workspace_handle_v1_destroy(workspace->priv->handle);
...
xfw_workspace_wayland_activate(XfwWorkspace *workspace, GError **error) {
    ...
    if ((wworkspace->priv->capabilities & XFW_WORKSPACE_CAPABILITIES_ACTIVATE) != 0) {
        zcosmic_workspace_handle_v1_activate(XFW_WORKSPACE_WAYLAND(workspace)->priv->handle);
        return TRUE;
    } else {
        if (error != NULL) {
...
xfw_workspace_wayland_remove(XfwWorkspace *workspace, GError **error) {
    ...
    if ((wworkspace->priv->capabilities & XFW_WORKSPACE_CAPABILITIES_REMOVE) != 0) {
        zcosmic_workspace_handle_v1_remove(XFW_WORKSPACE_WAYLAND(workspace)->priv->handle);
        return TRUE;
    } else {
        if (error != NULL) {
...
static void
workspace_name(void *data, struct zcosmic_workspace_handle_v1 *wl_workspace, const char *name) {
...
static void
workspace_coordinates(void *data, struct zcosmic_workspace_handle_v1 *wl_workspace, struct wl_array *coordinates) {
    XfwWorkspaceWayland *workspace = XFW_WORKSPACE_WAYLAND(data);
...
static const struct {
    enum zcosmic_workspace_handle_v1_state wl_state;
    XfwWorkspaceState state_bit;
} state_converters[] = {
    { ZCOSMIC_WORKSPACE_HANDLE_V1_STATE_ACTIVE, XFW_WORKSPACE_STATE_ACTIVE },
    { ZCOSMIC_WORKSPACE_HANDLE_V1_STATE_URGENT, XFW_WORKSPACE_STATE_URGENT },
    { ZCOSMIC_WORKSPACE_HANDLE_V1_STATE_HIDDEN, XFW_WORKSPACE_STATE_HIDDEN },
};

static void
workspace_state(void *data, struct zcosmic_workspace_handle_v1 *wl_workspace, struct wl_array *wl_state) {
    XfwWorkspaceWayland *workspace = XFW_WORKSPACE_WAYLAND(data);
    ...
    enum zcosmic_workspace_handle_v1_state *state;
    wl_array_for_each(state, wl_state) {
        for (gsize i = 0; i < G_N_ELEMENTS(state_converters); ++i) {
            if (state_converters[i].wl_state == *state) {
...
static const struct {
    enum zcosmic_workspace_handle_v1_zcosmic_workspace_capabilities_v1 wl_capability;
    XfwWorkspaceCapabilities capability_bit;
} capabilities_converters[] = {
    { ZCOSMIC_WORKSPACE_HANDLE_V1_ZCOSMIC_WORKSPACE_CAPABILITIES_V1_ACTIVATE, XFW_WORKSPACE_CAPABILITIES_ACTIVATE },
    { ZCOSMIC_WORKSPACE_HANDLE_V1_ZCOSMIC_WORKSPACE_CAPABILITIES_V1_DEACTIVATE, XFW_WORKSPACE_CAPABILITIES_DEACTIVATE },
    { ZCOSMIC_WORKSPACE_HANDLE_V1_ZCOSMIC_WORKSPACE_CAPABILITIES_V1_REMOVE, XFW_WORKSPACE_CAPABILITIES_REMOVE },
};

static void
workspace_capabilities(void *data, struct zcosmic_workspace_handle_v1 *wl_workspace, struct wl_array *wl_capabilities) {
    XfwWorkspaceWayland *workspace = XFW_WORKSPACE_WAYLAND(data);
    ...
    enum zcosmic_workspace_handle_v1_zcosmic_workspace_capabilities_v1 *cap;
    wl_array_for_each(cap, wl_capabilities) {
        for (gsize i = 0; i < G_N_ELEMENTS(capabilities_converters); ++i) {
            if (capabilities_converters[i].wl_capability == *cap) {
...
static void
workspace_removed(void *data, struct zcosmic_workspace_handle_v1 *wl_workspace) {
    XfwWorkspaceWayland *workspace = XFW_WORKSPACE_WAYLAND(data);
