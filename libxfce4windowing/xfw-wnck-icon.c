                                 ...
                                 GError **error)
{
    ...
    if (G_LIKELY(window_icons != NULL)) {
        ...
        if (error != NULL) {
            *error = g_error_new(XFW_ERROR, XFW_ERROR_ICON, _("The provided window does not have a _NET_WM_ICON or WMHints icon"));
        }
        return FALSE;
    }
}
                   ...
                   GError **error)
{
    ...
    if (G_LIKELY(window_icon != NULL)) {
        ...
        if (error != NULL) {
            *error = g_error_new_literal(XFW_ERROR, XFW_ERROR_ICON, _("Failed to find or load an icon for the window"));
        }
        return NULL;
    }
}
