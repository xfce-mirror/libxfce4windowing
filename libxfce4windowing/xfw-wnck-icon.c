...
static GObject *xfw_wnck_icon_constructor(GType type,
...
xfw_wnck_icon_class_init(XfwWnckIconClass *klass) {
    ...
    gobject_class->constructor = xfw_wnck_icon_constructor;
...
static GObject *
xfw_wnck_icon_constructor(GType type,
                          guint n_construct_properties,
                          GObjectConstructParam *construct_properties)
{
    GObject *wnck_object = NULL;
    GList *window_icons;

    for (guint i = 0; i < n_construct_properties; ++i) {
        if (g_strcmp0(g_param_spec_get_name(construct_properties[i].pspec), "wnck-object") == 0) {
            wnck_object = g_value_get_object(construct_properties[i].value);
        }
    }
    ...
    if (G_LIKELY(window_icons != NULL)) {
        GObject *object = G_OBJECT_CLASS(xfw_wnck_icon_parent_class)->constructor(type, n_construct_properties, construct_properties);
...
_xfw_wnck_icon_new(GObject *wnck_object)
{
    ...
    return  g_object_new(XFW_TYPE_WNCK_ICON,
