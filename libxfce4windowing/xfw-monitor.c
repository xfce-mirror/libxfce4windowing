...
static GOnce pnp_ids_init = G_ONCE_INIT;
...
chop_end(gchar *str, const gchar *chop_str) {
    gchar *to_chop = g_strrstr(str, chop_str);

    if (to_chop == NULL || to_chop[strlen(chop_str)] != '\0') {
        return FALSE;
    } else {
        *to_chop = '\0';
        return TRUE;
...
static gchar *
chop_unneeded_suffixes(gchar *str) {
    for (gsize j = 0; j < G_N_ELEMENTS(unneeded_suffixes) ; ++j) {
        if (chop_end(str, unneeded_suffixes[j])) {
            break;
        }
    }
...
init_pnp_ids(gpointer data) {
    ...
    if (g_file_get_contents(PNP_IDS_PATH, &contents, NULL, &error)) {
        ...
        for (gsize i = 0; lines[i] != NULL; ++i) {
            ...
            if (tab != NULL && tab[1] != '\0') {
                *tab = '\0';
                g_hash_table_insert(pnp_ids, lines[i], chop_unneeded_suffixes(tab + 1));
            }
        }
        ...
        g_message("Failed to load %s (%d): %s", PNP_IDS_PATH, error->code, error->message);
        g_error_free(error);
    }
...
xfw_monitor_get_vendor_name(const gchar *vendor_code) {
    g_once(&pnp_ids_init, init_pnp_ids, NULL);
    ...
    if (priv->friendly_name == NULL) {
                ...
                || g_strcmp0(model, "PANEL") == 0))
        {
            ...
            if (manufacturer_name != NULL) {
                ...
                manufacturer = chop_unneeded_suffixes(g_strdup(manufacturer_code));
            } else {
