...
xfw_window_x11_get_monitors(XfwWindow *window) {
    ...
    if (xwindow->priv->monitors == NULL) {
        ...
        // NB: this only works on X11, as X11 can have only one scale factor for all monitors
        gint scale_factor = gdk_monitor_get_scale_factor(gdk_display_get_monitor(display, 0));
        GdkRectangle *geom = &xwindow->priv->geometry;
        gint points[4][2] = {
            { geom->x / scale_factor,                     geom->y / scale_factor },
            { (geom->x + geom->width - 1) / scale_factor, geom->y / scale_factor },
            { geom->x / scale_factor,                     (geom->y + geom->height - 1) / scale_factor },
            { (geom->x + geom->width - 1) / scale_factor, (geom->y + geom->height - 1) / scale_factor },
        };
