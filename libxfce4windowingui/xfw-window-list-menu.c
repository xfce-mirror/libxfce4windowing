...
xfw_window_list_menu_class_init(XfwWindowListMenuClass *klass) {
    ...
    /**
     * XfwWindowListMenu:minimized-icon-lucency:
     *
     * The lucency of icons for minimized windows.  The value should be between
     * 0 and 100.  Lower values will make the icon look more like a greyscale
     * image.
     **/
    g_object_class_install_property(gobject_class,
                                    PROP_MINIMIZED_ICON_LUCENCY,
                                    g_param_spec_int("minimized-icon-lucency",
                                                     ...
                                                     "Lucency (inverse greyscale-ness) of minimized window icons",
                                                     MIN_MINIMIZED_ICON_LUCENCY,
                                                     MAX_MINIMIZED_ICON_LUCENCY,
...
xfw_window_list_menu_init(XfwWindowListMenu *menu) {
    ...
    menu->minimized_icon_lucency = DEFAULT_MINIMIZED_ICON_LUCENCY;
...
xfw_window_list_menu_set_property(GObject *object, guint property_id, const GValue *value, GParamSpec *pspec) {
    ...
    switch (property_id) {
        ...
        case PROP_MINIMIZED_ICON_LUCENCY:
            menu->minimized_icon_lucency = CLAMP(g_value_get_int(value),
                                                 MIN_MINIMIZED_ICON_LUCENCY,
                                                 MAX_MINIMIZED_ICON_LUCENCY);
            break;
...
xfw_window_list_menu_get_property(GObject *object, guint property_id, GValue *value, GParamSpec *pspec) {
    ...
    switch (property_id) {
        ...
        case PROP_MINIMIZED_ICON_LUCENCY:
            g_value_set_int(value, menu->minimized_icon_lucency);
            break;
...
add_window_menu_item(XfwWindowListMenu *menu,
    ...
    if (menu->show_icons) {
        ...
        if (icon != NULL) {
            ...
            if (menu->minimized_icon_lucency < 100
                && (xfw_window_is_minimized(window) || xfw_window_is_shaded(window)))
            {
                ...
                gdk_pixbuf_saturate_and_pixelate(icon, icon, (gdouble)menu->minimized_icon_lucency / 100.0, TRUE);
            }
