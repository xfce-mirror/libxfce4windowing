...
add_workspace_header(XfwWindowListMenu *menu, XfwWorkspaceGroup *group, gint group_number, XfwWorkspace *workspace) {
    ...
    gchar *ws_label;
    const gchar *ws_name = xfw_workspace_get_name(workspace);
    if (ws_name == NULL || ws_name[0] == '\0') {
        ...
        ws_label = g_markup_escape_text(ws_name, strlen(ws_name));
    }
...
add_window_menu_item(XfwWindowListMenu *menu,
    ...
    gchar *title;
    const gchar *window_title = xfw_window_get_name(window);
    if (window_title != NULL) {
        title = g_markup_escape_text(window_title, -1);
    } else {
        title = g_markup_escape_text(_("(Unnamed Window)"), -1);
    }
    ...
    if (menu->show_icons) {
        ...
        if (icon != NULL) {
            ...
            if (xfw_window_is_minimized(window) || xfw_window_is_shaded(window)) {
                ...
                gdk_pixbuf_saturate_and_pixelate(icon, icon, 0.55, TRUE);
            }
...
populate_window_list_menu(XfwWindowListMenu *menu) {
    ...
    for (GList *gl = groups; gl != NULL; gl = gl->next, ++group_num) {
        ...
        if (menu->show_urgent_windows_section && urgent_windows != NULL) {
            add_separator_item(GTK_MENU(menu));
        ...
        if (menu->show_workspace_actions) {
            ...
            if (can_remove) {
                ...
                if (ws_name == NULL) {
                    ...
                    label_text = g_strdup_printf(_("_Remove Workspace '%s'"), ws_name_esc);
                    g_free(ws_name_esc);
                }
