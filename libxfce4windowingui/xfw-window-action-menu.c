...
static void screen_monitors_changed(XfwScreen *screen, XfwWindowActionMenu *menu);
...
xfw_window_action_menu_constructed(GObject *obj) {
    ...
    g_signal_connect(screen, "monitors-changed", G_CALLBACK(screen_monitors_changed), menu);
...
xfw_window_action_menu_dispose(GObject *obj) {
    ...
    if (menu->priv->screen != NULL) {
        g_signal_handlers_disconnect_by_func(menu->priv->screen, screen_monitors_changed, menu);
        menu->priv->screen = NULL;
    }
...
update_move_monitor_submenu(XfwWindowActionMenu *menu) {
    GList *monitors = xfw_screen_get_monitors(menu->priv->screen);
...
static void
screen_monitors_changed(XfwScreen *screen, XfwWindowActionMenu *menu) {
    update_menu_items(menu);
}
